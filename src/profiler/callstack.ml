...
module CallStack =
  struct
    type elt = { stack : (string * int) list; index : int }
    ...
    let push (cstack : t) (tid : int) (faddr : string) : unit =
      ...
      in
        ...
        | [] ->
            Hashtbl.replace cstack tid { stack = [ (faddr, 1) ]; index = 1; };
						 Printf.printf "Add: %s" (string_of_callstack cstack tid); 
        | (k, i) :: tl ->
            Hashtbl.replace cstack tid
              {
                ...
                index = value.index + 1;
							...
							Printf.printf "Remove: %s" (string_of_callstack cstack tid)
    ...
    let pop (cstack : t) (tid : int) : (string * int) =
      ...
      in
        ...
        | [] -> ("", 0)
        | (k, i) :: tl ->
            (Hashtbl.replace cstack tid { (value) with stack = tl; };
             Printf.printf "Remove: %s" (string_of_callstack cstack tid); 
             (k, i))
